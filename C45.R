...
m1 <- J48(Continent~.,control = Weka_control(R = FALSE , C = 0.5 , M = 0),data = trainInput1)

set.seed(2909)
trainIndex <- sample(2,nrow(inputset),replace = TRUE,prob=c(0.8,0.2))
trainInput2 <- inputset[trainIndex==1,]
testInput2 <- inputset[trainIndex==2,]
m2 <- J48(Continent~.,control = Weka_control(R = FALSE , C = 0.5 , M = 3), data = trainInput2)

set.seed(386)
trainIndex <- sample(2,nrow(inputset),replace = TRUE,prob=c(0.8,0.2))
trainInput3 <- inputset[trainIndex==1,]
testInput3 <- inputset[trainIndex==2,]
m3 <- J48(Continent~.,control = Weka_control(R = FALSE , C = 0.5 , M = 3), data = trainInput3)

set.seed(7900)
trainIndex <- sample(2,nrow(inputset),replace = TRUE,prob=c(0.8,0.2))
trainInput4 <- inputset[trainIndex==1,]
testInput4 <- inputset[trainIndex==2,]
m4 <- J48(Continent~.,control = Weka_control(R = FALSE , C = 0.5, M = 0), data = trainInput4)

set.seed(9642)
trainIndex <- sample(2,nrow(inputset),replace = TRUE,prob=c(0.8,0.2))
trainInput5 <- inputset[trainIndex==1,]
testInput5 <- inputset[trainIndex==2,]
m5 <- J48(Continent~.,control = Weka_control(R = FALSE , C = 0.4 , M = 0), data = trainInput5)
